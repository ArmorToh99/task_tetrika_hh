...
(начиная с 0) время входа на урок, под нечетными - время выхода с урока. Нужно написать функцию appearance, которая
...
def appearance(intervals: dict[str, list[int]]) -> int:
    """
    Функция для вычисления общего времени присутствия ученика и учителя на уроке (в секундах).
    Возвращает целое число
    """
    def time_calculating(dict_name):
        """
        Внутренняя функция для получения значений секунд заданного интервала времени.
        Возвращает множество, содержащее секунды заданного интервала времени
        """
        in_time = dict_name[::2]
        out_time = dict_name[1::2]
        all_time = []
        for a, b in zip(in_time, out_time):
            for item in range(a, b):
                all_time.append(item)
        return set(all_time)

    return len(time_calculating(intervals['pupil']) & time_calculating(intervals['tutor']) & time_calculating(intervals['lesson']))
